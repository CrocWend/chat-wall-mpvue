    ...
    <div class="container-home">
        ...
        <div v-if="!isSign">
            ...
            <van-cell-group>
                ...
                <van-field :value="signMessage" required label="留言" type="text" title-width="70px" placeholder="msgPlaceholder" border="false" @input="bindContentInput" />
            </van-cell-group>
            <!-- 输入手机号模式 不再调用授权 -->
            ...
            <div v-else>
                <van-button class="button" custom-class="button" size="large" type="primary" open-type="getPhoneNumber" bind:getphonenumber="getPhoneNumber">先签到</van-button>
            </div>
        </div>
        <!-- 加入群聊按钮 已签到显示 -->
        <div v-else>
            <van-button class="button" block size="large" type="primary" open-type="getUserInfo" bindgetuserinfo="bindGetUserInfo">加入群聊</van-button>
        </div>
...
<script>
var app = getApp();
import Toast from '../../../modules/vant-ui/toast/toast';
...
export default {
    ...
    methods: {
        ...
        bindPhoneInput: function(e) {
            this.signPhone = e.detail
        },
        bindContentInput: function(e) {
            this.signMessage = e.detail
        },
        bindGetUserInfo(e) {
            this.joinRoom(app.globalData.appInfo.phone, e.detail.userInfo)
        },
        /**
         * 获取用户手机号
         * 成功后 调用签到
            ...
            wx.request({
                ...
                success(res) {
                    ...
                    if (data.result) {
                        ...
                        // 设置本地存储
                        try {
                            wx.setStorageSync('appInfo', app.globalData.appInfo)
                        } catch (e) {}
